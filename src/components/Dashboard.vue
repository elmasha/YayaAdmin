...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      let productDataSet = await db_real.ref("/charts/Bureau_chart").once("value");
  ...
  methods: {
    ...
    showCharts() {
      var tutorialsRef = db_real.ref("/charts/Bureau_chart");
      tutorialsRef.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
          // var key = childSnapshot.key;
          // var data = childSnapshot.val();
          tutorials.push(JSON.stringify(childSnapshot.val()));
          chart = tutorials[0];
          console.log("Real_time", chart);
        });
      });
    },
  },
